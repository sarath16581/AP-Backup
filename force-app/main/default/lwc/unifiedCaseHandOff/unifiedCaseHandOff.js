...
import getBusinessUnit from "@salesforce/apex/UnifiedCaseHandOffController.getBusinessUnit";
import handOffCase from "@salesforce/apex/UnifiedCaseHandOffController.handOffCase";
import searchConsignmentStarTrack from "@salesforce/apexContinuation/UnifiedCaseHandOffController.searchConsignmentStarTrack";
...
export default class UnifiedCaseHandOff extends LightningElement {
	isLoading = false;
	@api recordId;
	errorMessage;
	@track caseRecord;
	...
	wiredCase({ error, data }) {
		if (data) {
			...
			this.handoffWrapper.caseRecordType = this.caseRecord.fields.RecordType.value.fields.DeveloperName.value;
		} else if (error) {
	...
	wiredCasePicklists({ error, data }) {
		if (data) {
			this.handoffReasonOptions = data.picklistFieldValues.CaseTransferReason__c.values;
		} else if (error) {
			this.isLoading = false;
			this.showInternalErrorToast();
			...
			if (this.handoffWrapper.businessUnit === "StarTrack") {
				searchConsignmentStarTrack({ consignmentNumber: this.articleName })
					.then((result) => {
						...
						if (result.length > 1) {
							this.isLoading = false;
							this.showNotification("Error", "Case transfer failed as consignment search found duplicate records, report the issue to your manager", "error");
						} else {
					...
					.catch((error) => {
						this.isLoading = false;
						this.showNotification("Error", "Consignment search failed. Please contact Salesforce support team", "error");
						console.error("searchConsignmentStarTrack error ==>", JSON.stringify(error));
					});
			} else {
	...
	processHandOffCase() {
		...
		handOffCase({
			...
			.then((result) => {
				this.isLoading = false;
				this.showNotification("Success", "Case transferred successfully", "success");
				this.dispatchEvent(new CloseActionScreenEvent());
				this.closeCaseTab();
			})
			.catch((error) => {
				this.isLoading = false;
				this.showNotification("Error", "Case transfer failed, please try again. If the error continues, report the issue to your manager", "error");
	...
	checkValidity() {
		let isInputsCorrect = [...this.template.querySelectorAll("lightning-combobox, lightning-textarea")].reduce((validSoFar, inputField) => {
	...
	showInternalErrorToast() {
		this.showNotification("Error", "Something went wrong. Please contact Salesforce support team", "error");
	}
