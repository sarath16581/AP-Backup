...
define(['util', 'moment', 'integration', 'i18next', 'config', 'SFDC/tracking'],
    ...
    var start = function(params) {
        ...
        if (config.CREATE_TASK && config.NEW_TASK_ON_POP && !popOnly) {
            ...
            ConnectorController.createTask(contact.Id, whatId, subject, ixn.userData, taskMap,
                function (task) {
                    if (task !== null) {
    ...
    var finish = function(id, comments, ixn, duration) {
        ...
        if (config.CREATE_TASK && ixn.parentCallUri === undefined) { // don't do tasks with consult calls
            ...
            ConnectorController.closeTask(taskId, whatId, subject, comments, ixn.userData, config.DISPOSITION_KVP,
                        taskMap, duration,
                function (result) {
                    if (result !== null) {
