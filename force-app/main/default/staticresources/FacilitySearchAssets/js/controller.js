...
//		    console.log('>> markerGroups: ', markerGroups);
		...
		getCodeAddress: function(e) {
			...
			// grab the international country name and pass it
			var countryName = j$('[id$="countryInp"]').val();
			...
			geocoder.geocode({'address': address}, function(results, status) {
				if (status == google.maps.GeocoderStatus.OK) {
					...
					doSearchAddress(locationParameter, countryName);
				} else {
					my.hideProgress();
		...
		contactFacility: function(method, facilityId, facilityEmail, facilityEscalationEmail, el) {
			...
			if(method == 'via ServiceNow' && model.get('contactEmail') == '') {
				...
				if(model.get('requiresToSetEmail') == 'true') {
					FacilitySearchController.setTempEmailToContact(model.get('caseContactId'), function(result, event) {
		...
		contactViaEmail: function(deprecated,facilityID, el, preventStatusChange, contactRequiresEscalation, escalationEmail) {
			...
			FacilitySearchController.contactViaEmail(comments,facilityID, model.get('caseId'), preventStatusChange, contactRequiresEscalation, escalationEmail, function(returnedCaseId, event) {
				...
				if(event.status){
				    if(returnedCaseId.indexOf('Error') == 0){
						...
						if (sforce.console.isInConsole()) {
							sforce.console.getEnclosingTabId(function(result) {
								sforce.console.getEnclosingPrimaryTabId(function(r) {
									...
									sforce.console.refreshPrimaryTabById(r.id, true, null, true);
								});
							});
		...
		contactViaFax: function(facilityId, preventStatusChange, el) {
			...
			FacilitySearchController.contactViaFax(facilityId, model.get('caseId'), faxComments, preventStatusChange, function(result, event){
		...
		contactViaSalesforce: function(escalationEmail, facilityId, el, preventStatusChange) { //, el){
			...
			FacilitySearchController.contactViaSalesforce(model.get('caseId'), escalationEmail, facilityId, preventStatusChange, function(result, event) {
				hideFacilitySearchProgress();
				...
				if(event.status) {
					if(result.indexOf('Error:') == 0) {
						...
						if (sforce.console.isInConsole()) {
							...
							//alert("Email is sent successfully"); Commented as per REQ1495049
							alert("Case sent successfully to MyNetwork facility");
						} else {
							//window.open(emailURL);
							 //alert("Email is sent successfully"); Commented as per REQ1495049
							 alert("Case sent successfully to MyNetwork facility");
		...
		lodgementPoint: function(facilityId){
			...
			FacilitySearchController.lodgementPoint(model.get('caseId'), facilityId, function(result, event){
				my.hideProgress();
				sforce.console.getEnclosingTabId(function(result) {
					sforce.console.getEnclosingPrimaryTabId(function(r) {
						sforce.console.closeTab(result.id);
		...
		contactViaManually: function(facilityId, preventStatusChange, externalSystem){
			if(!preventStatusChange) {
				FacilitySearchController.contactViaManually(model.get('caseId'), facilityId, externalSystem, function(result, event) {
					hideFacilitySearchProgress();
					...
					if(event.status) {
						...
						alert('System Error: ' + event.message + '=====>>>>' + event.where);
					}
				});
			} else {
				hideFacilitySearchProgress();
		...
		toggleRelatedList: function(uniqueId, title, objectId, relatedFieldNames, isCaseRelated) {
			var uniqueIds = {};

			var $a = j$('#toggler_' + uniqueId);
			var showFrame = true;
			if($a.text().indexOf('Hide') == 0) showFrame = false;

			if(j$('#myframe_' + uniqueId).length == 0 && showFrame) {
	...
	return {
		...
		toggleRelatedList: my.toggleRelatedList,
