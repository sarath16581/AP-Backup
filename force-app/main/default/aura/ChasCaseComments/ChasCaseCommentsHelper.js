({
	...
	//INC2216801 - Workaround to fix H&S users getting insufficient access when they close or reopen closed case.
	updateCaseStatus : function(cmp, newStatus, callback, helper) {        
		...
		action.setParams({ "caseToUpdate" : vcase });
		action.setCallback(this, function(response) {
			var state = response.getState();
			if (state === "SUCCESS") {
				console.log("Save completed successfully.");
				cmp.find('caseRecordData').reloadRecord(true);
				cmp.getEvent("updateCaseStatus").fire();
				if(newStatus === "Closed") {
					helper.pushAnalyticsEvent(cmp, 'case', 'closed'); //analytics track after a case has been successfully closed
				}
			} else if (state === "INCOMPLETE") {
