 ...
 ({
    ...
    checkEachInput: function(cmp, inputs, showError) {
        ...
        if (cmp.get('v.wizardData.selectedRadio1Name') === 'Mail products' &&
            ...
            if ($A.util.isEmpty(selectedPostOffice) || $A.util.isUndefined(selectedPostOffice)) {
                isValid = false;
               errors.push({name: 'preferredLocation', label: 'Preferred post office for collection', error: ''});
            }
        }
        ...
        if (cmp.get('v.wizardData.selectedRadio1Name') === 'Online Shop' &&
    ...
    updateErrorSummary: function(cmp, allInputs) {
        ...
        if (cmp.get('v.wizardData.selectedRadio1Name') === 'Mail products' &&
            cmp.get('v.wizardData.mailProductsEnquiryType') === 'Transfer to another post office') {
        if ($A.util.isEmpty(selectedDeliveryAddress) || $A.util.isUndefined(selectedDeliveryAddress)) {
            errors.push({name: 'AMEOriginalDeliveryAddress', label: 'Original delivery address', error: ''});
        }
        }
        if (cmp.get('v.wizardData.selectedRadio1Name') === 'Online Shop' &&
            cmp.get('v.wizardData.selectedRadio2Name') === 'Yes') {
