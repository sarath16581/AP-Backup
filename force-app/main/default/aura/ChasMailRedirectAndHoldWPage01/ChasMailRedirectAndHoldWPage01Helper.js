...
({
    ...
    //     cmp.set('v.errors', []);
    //     // Remove Add Person Error
    //     var addPersonErrorCmp = cmp.find('addPersonError');
    //     if (addPersonErrorCmp) addPersonErrorCmp.set('v.error', null);

    //     var isValid = true;
    //     var household = cmp.get("v.wizardData.household");
    //     var givenNames = this.asArray(cmp.find("givenName"));
    //     var surnames = this.asArray(cmp.find("surname"));

    //     for (var i in household) {
    //         isValid =  isValid & (
    //             this.validateGivenName(givenNames[i], showError) &
    //             this.validateSurname(surnames[i], showError)
    //             );         
    //     }
    //     // Must use single '&' so that it runs through all functions.
    //     isValid = isValid & (
    //         this.validateRadioButtons(cmp.find("enquiryTypeRadioButtons"), showError) &
    //         this.validateRadioButtons(cmp.find("emailOrPhoneRadioButtons"), showError) &
    //         this.validateRadioButtons(cmp.find("parcelsOrLettersRadioButtons"), showError) &
    //         this.validateRadioButtons(cmp.find("withinAusOrOverseasRadioButtons"), showError) &
    //         this.validateEmail(cmp.find("emailAddress"), showError) &
    //         this.validatePhone(cmp.find("phoneNumber"), showError) &
    //         this.validateAddress(cmp.find("addressLine1"), showError) &
    //         this.validateCity(cmp.find("city"), showError) &
    //         this.validateState(cmp.find("state"), showError) &
    //         this.validatePostcode(cmp.find("postcode"), showError) &
    //         this.validateCountry(cmp.find("newCountry"), showError) &
    //         this.validateAddress(cmp.find("newAddressLine1"), showError) &
    //         this.validateCity(cmp.find("newCityAU"), showError) &
    //         this.validateState(cmp.find("newStateAU"), showError) &
    //         this.validatePostcode(cmp.find("newPostcodeAU"), showError) &
    //         this.validateEnquiryDetails(cmp.find("enquiryDetails"), showError) &
    //         this.validateCondition(cmp.find("conditionCheckbox"), showError)
    //         );
    //     if (isValid) {
    //         cmp.set('v.formValid', true);
    //     }else{
    ...
    //     return isValid;
    // },
    // checkAddPersonIsValid: function(cmp,givenName, surame, showError) { 
    //     console.log('givenName='+givenName);
    //     console.log('surame='+surame);
    //     // Must use single '&' so that it runs through all functions.
    //     var isValid =  (
    //         this.validateSurname(cmp.find(surame), showError)&
    //         this.validateGivenName(cmp.find(givenName), showError)
    //     );
    //     return isValid;
    // },
