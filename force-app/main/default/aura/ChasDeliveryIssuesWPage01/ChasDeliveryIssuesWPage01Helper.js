...
({
			...
			action.setCallback(this, function (response) {
				...
				if (state === "SUCCESS") {
					...
					if (
						...
						cmp.set(
							"v.wizardData.latestDeliveredScanWcid",
							returnObj["trackingNumberDetails"][0]
								.latestDeliveredScanWcid
						);
						cmp.set(
							"v.wizardData.previousDeliveredScanWcid",
							returnObj["trackingNumberDetails"][0]
								.previousDeliveredScanWcid
						);
						cmp.set(
							"v.wizardData.duplicateCase",
							returnObj["trackingNumberDetails"][0].duplicateCase
						);
						cmp.set(
							"v.wizardData.isReturnToSender",
							returnObj["trackingNumberDetails"][0]
								.isReturnToSender
						);
						cmp.set(
	...
	checkNetworkEligibility: function (cmp, event, helper) {
		...
		var isReturnToSender = $A.util.isUndefined(
			cmp.get("v.wizardData.isReturnToSender")
		)
			? false
			: cmp.get("v.wizardData.isReturnToSender");
		var isParcelAwaitingCollection = $A.util.isUndefined(
			cmp.get("v.wizardData.isParcelAwaitingCollection")
		)
			? false
			: cmp.get("v.wizardData.isParcelAwaitingCollection");
		var attemptedDeliveryScanWcid = cmp.get(
			"v.wizardData.latestDeliveredScanWcid"
		);
		var previousDeliveredScanWcid = cmp.get(
			"v.wizardData.previousDeliveredScanWcid"
		);
		// set network eligibility flag based on issue type
		if (
			issueName == "Item was left in an unsafe place" ||
			issueName == "Postie didn't knock before leaving a card"
		) {
			// check for duplicate case, RTS, delivered scan
			if (
				duplicateCase == "" &&
				!isReturnToSender &&
				!$A.util.isEmpty(attemptedDeliveryScanWcid)
			) {
		...
		if (
			...
			if (
				duplicateCase == "" &&
				!isReturnToSender &&
				!$A.util.isEmpty(previousDeliveredScanWcid)
			) {
				isEligibleForMyNetworkAssignment = true;
			}
		}
		...
		cmp.set(
			"v.wizardData.isEligibleForNetworkAssignmentDeliveryIssue",
			isEligibleForMyNetworkAssignment
		);
	},
	...
	checkEachInput: function (cmp, inputs, showError) {
		...
		var incorrectDeliveryAddress = cmp.get(
			"v.wizardData.incorrectDeliveryAddress"
		);
		var IssueName = cmp.get("v.wizardData.IssueName");
		...
		if (IssueName == "Incorrect delivery address needs fixing") {
			if (
				$A.util.isEmpty(incorrectDeliveryAddress) ||
				$A.util.isUndefined(incorrectDeliveryAddress)
			) {
				isValid = false;
			}
		}
	...
	updateErrorSummary: function (cmp, allInputs) {
		...
		if (IssueName == "Incorrect delivery address needs fixing") {
			if (
				...
				errors.push({
					name: "AMEIncorrectDeliveryAddress",
