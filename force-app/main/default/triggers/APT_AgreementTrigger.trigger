...
2023-06-29 - NAsir Jawed - Added code to sendEmailToDriver() to send mail to Opportunity Driver when Agreement signed date cross Proposal Expiry date
2023-07-17 - Yatika Bansal - Updated code to call agreementDatesLogicOnCreate after setProductLines
*/
...
trigger APT_AgreementTrigger on Apttus__APTS_Agreement__c (after insert,before insert,before update,after update, before delete, after delete, after undelete) {
	...
	if (Trigger.isAfter) {
		...
		//update
			...
			APT_AgreementTriggerHandler.sendEmailToDriver(Trigger.oldMap, Trigger.newMap);
