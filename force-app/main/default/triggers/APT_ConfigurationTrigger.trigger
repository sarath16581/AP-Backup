...
trigger APT_ConfigurationTrigger on Apttus_Config2__ProductConfiguration__c (before update, after insert, after update, before delete) {
    ...
    if(trigger.isAfter && trigger.isupdate){
        ...
        for(Apttus_Config2__ProductConfiguration__c proConfig : trigger.new){
            if(((trigger.oldmap.get(proConfig.id).Apttus_Config2__Status__c!=proConfig.Apttus_Config2__Status__c 
                ...
                ('Saved'.equalsIgnoreCase(proConfig.Apttus_Config2__Status__c) && 
                    trigger.oldmap.get(proConfig.id).Apttus_Config2__NumberOfItems__c != proConfig.Apttus_Config2__NumberOfItems__c)) && proConfig.Dov_Type__c != 'Decrease Revenue'){
        ...
        System.debug('----> Config Id Set in Scope--->'+prodConfigIdSet +'-----> Quote Id Set ---->' + quoteIdSet);
        ...
        /* Shashwat.Nath@Auspost.com has commented the below method call as the Sync process has been moved to 
           an asynchronous queable process above */
        //APT_ConfigurationTriggerHandler.syncWithOpportunity(trigger.new,trigger.oldMap);
    }
