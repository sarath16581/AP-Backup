...
2023-07-05 - Ranjeewa Silva - Added new Module framework based trigger dispatch LeadAutomationTriggerHandler
**************************************************/

trigger LeadTrigger on Lead (after delete, after undelete, before insert, before update, after insert, after update) {

	if(!TriggerHelper.isTriggerDisabled(String.valueOf(Lead.SObjectType))) {	 // verify if triggers are disabled
		new LeadAutomationTriggerHandler().dispatch();
	}
